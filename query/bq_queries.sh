...
bq query --project proj-wikimeasure "SELECT  count(*), claimant_address, integer(sum(float(settlement_amount  ))) settlement, integer(sum(float(claimed_amount   ))) claimed FROM [analysis.nyc_comptroller_claims] group by claimant_address order by 1 desc limit 10"
